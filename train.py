...
def _dataloader_kwargs(cfg, device, shuffle: bool, sampler=None):
    kw = {
...
def _state_dict(model):
    return _model_core(model).state_dict()


def _load_state_dict(model, state_dict, strict=False):
    return _model_core(model).load_state_dict(state_dict, strict=strict)
...
def evaluate(model, tokenizer, dataloader, device, cfg, cls_loss_fn, use_amp, quiet=False):
    model.eval()
    y_true, y_pred = [], []
    total_loss = 0.0
    total_exp_loss = 0.0
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating", leave=False, disable=quiet):
            ...
            with autocast_cm:
                ...
                exp_loss = (
                    compute_explainer_loss(
                        _model_core(model), tokenizer, batch, device, cfg, outputs
                    )
    ...
    *,
    ...
    if distributed:
        if not torch.cuda.is_available():
            raise RuntimeError("torchrun/DDP requires CUDA, but torch.cuda.is_available() is False.")
        torch.cuda.set_device(local_rank)
        torch.distributed.init_process_group(backend="nccl")
    quiet = quiet or not _is_main_process()
    ...
    try:
        ...
        if getattr(cfg, "USE_ENCODER_GRADIENT_CHECKPOINTING", False):
            if distributed:
                # DDP already shards the batch; avoid reentrant checkpoint hooks on the
                # shared GraphCodeBERT encoder used by augmentation/explainer branches.
                try:
                    _model_core(model).encoder.roberta.gradient_checkpointing_disable()
                except AttributeError:
                    pass
            else:
                _model_core(model).encoder.roberta.gradient_checkpointing_enable()
        ...
        train_sampler = (
            DistributedSampler(train_set, shuffle=True, seed=cfg.RANDOM_SEED)
            if distributed
            else None
        )

        train_loader = DataLoader(
            train_set, **_dataloader_kwargs(cfg, device, shuffle=True, sampler=train_sampler)
        )
        ...
        for epoch in range(cfg.EPOCHS):
            ...
            for step, batch in enumerate(loop):
                ...
                with autocast_cm:
                        ...
                        if cfg.USE_INFONCE_VIEWS and aug_outputs:
                            ...
                            for i in range(len(zs)):
                                for j in range(i + 1, len(zs)):
                                    losses.append(
                                        infonce_symmetric(zs[i], zs[j], cfg.TAU_CONTRASTIVE)
                                    )
                            l_con = (
                    ...
                    l_clu = (
                        _model_core(model).dec_clustering_loss(
                            contract_cls, cfg.CLUSTER_LOSS_TEMP
                        )
            ...
            val_metrics = evaluate(
                model, tokenizer, val_loader, device, cfg, cls_loss_fn, use_amp, quiet=quiet
            )
            ...
            if improved:
                ...
                patience_ctr = 0
                if _is_main_process():
                    torch.save(_state_dict(model), os.path.join(cfg.CKPT_DIR, "best_model.pt"))
            else:
                patience_ctr += 1

            should_stop = (
                cfg.EARLY_STOPPING_PATIENCE > 0
                and patience_ctr >= cfg.EARLY_STOPPING_PATIENCE
            )
            _barrier()
            if should_stop:
                if not quiet:
                    print(
                        f"Early stopping at epoch {epochs_ran} "
        ...
        if run_test and os.path.isfile(os.path.join(cfg.CKPT_DIR, "best_model.pt")):
            ...
            eval_model = _model_core(model)
            test_metrics = evaluate(
                eval_model, tokenizer, test_loader, device, cfg, cls_loss_fn, use_amp, quiet=quiet
            )
